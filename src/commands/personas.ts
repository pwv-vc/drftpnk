...
export function registerPersonasCommand(program: Command): void {
  ...
  personasCmd
    ...
    .action((id: string) => {
      try {
        ...
        console.log(pc.white('Signature devices:') + '  ' + persona.style.signature_devices.join(', '))
  ...
  personasCmd
    ...
    .action(async (id: string, opts) => {
      try {
        ...
        const updates = await analyzeStyleWithLLM(persona.name, persona.description, text, llmConfig)
  ...
  personasCmd
    ...
    .action(async () => {
      ...
      if (p.isCancel(urlsInput)) { p.cancel('Cancelled.'); process.exit(0) }
      ...
      if (p.isCancel(setDefault)) { p.cancel('Cancelled.'); process.exit(0) }
