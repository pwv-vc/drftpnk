  ...
  program
    ...
    .action(async (ideaFile: string, opts) => {
      try {
        ...
        const imagePrompt = await generateImagePrompt(idea, persona, plugin, config, context, debug)
        ...
        debug('dimensions:', imageResult.width && imageResult.height ? `${imageResult.width}×${imageResult.height}` : 'not reported by model')
