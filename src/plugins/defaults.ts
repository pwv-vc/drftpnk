...
export function resolveSystemPrompt(persona: { system_prompt?: string; style: { voice: string[]; signature_devices: string[]; tone_rules: string[] }; do_not?: string[] }): string {
  ...
  if (persona.style.signature_devices.length) parts.push(`Signature devices: ${persona.style.signature_devices.join(', ')}`)
