...
export function resolveOutlinePromptSources(
  persona: Persona,
  plugin: ContentTypePlugin,
): PromptSources {
...
export function resolveContentPromptSources(
  persona: Persona,
  plugin: ContentTypePlugin,
): PromptSources {
  const promptSet = persona.prompts?.[plugin.id];
...
export function buildContentPrompt(
  ...
  const { system: defaultSystem, user: defaultUser } = plugin.defaultContentPrompt(
    idea,
    persona,
    outline,
  );
...
export function resolveModelConfig(
  ...
  return {
    provider:
      byPersonaContentType.provider ??
      byPersona.provider ??
      byContentType.provider ??
      base.provider,
    model: byPersonaContentType.model ?? byPersona.model ?? byContentType.model ?? base.model,
    temperature:
      byPersonaContentType.temperature ??
      byPersona.temperature ??
      byContentType.temperature ??
      base.temperature,
    maxTokens:
      byPersonaContentType.maxTokens ??
      byPersona.maxTokens ??
      byContentType.maxTokens ??
      base.maxTokens,
  };
}
